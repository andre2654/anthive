...
must('painel à direita mostra a task e o estado', t.includes('task ─') && t.includes('in_progress') && g.actions.some(([key]) => key === 'd'));
const lines = t.split('\n'), ai = lines.findIndex((l) => l.includes('api') && l.includes('idle')), ti = lines.findIndex((l) => l.includes('◉ Corrigir ARBs'));
must('a tarefa pende do agente que a criou, com o galho', ai >= 0 && ti > ai && /[├╰]─/.test(lines[ti]!) && lines[ti]!.includes('in progress'));
