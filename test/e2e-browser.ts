...
const br = await P.addBrowser(p, 'oculto');
must('chrome oculto do anthive sobe', (await P.ensureBrowserUp(br)) === 'subiu');
...
must('mapa lê url e título do transcript', !!node && node.state.url.includes(`127.0.0.1:${port}`) && node.state.title === 'Loja da Ana');
