...
must('sem file: cria', (await ensureBus(d)) === 'new');
...
must('segunda vez não mexe', (await ensureBus(d)) === 'unchanged');
await Bun.write(join(d, '.mcp.json'), JSON.stringify({ mcpServers: { outro: { command: 'x' } } }));
must('arquivo alheio: acrescenta sem apagar', (await ensureBus(d)) === 'updated');
