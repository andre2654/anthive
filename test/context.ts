...
must('vira nó de arquivo com contagem de lines', !!node && node.lines === 4);
must('ligado ao agent sem ninguém pedir', v.edges.some((e) => e.from === a.id && e.to === node!.id && e.kind === 'context'));
const b = P.buildBriefing(v, 'db', 'revise');
must('briefing manda ler o CLAUDE.md primeiro', b.includes('CLAUDE.md (4 lines)') && b.includes('read it first'));
must('briefing não repete o contexto na lista de files', b.includes('none besides the context'));
