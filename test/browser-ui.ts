...
const br: BrowserNode = { kind: 'browser', id: 'b1', item: { kind: 'browser', id: 'b1', name: 'browser', mode: 'oculto', port: 9401, created: 0 }, state: { url: 'https://loja.dev/pedidos', title: 'Pedidos', snapshot: '- heading "Pedidos" [ref=e1]\n- link "Novo" [ref=e2]', console: '', busy: false, lastTool: 'browser_snapshot', live: true } };
...
const t = g.toString();
must('browser é caixa no mapa com modo, ao vivo e a página', t.includes('▣ chrome (oculto)') && t.includes('ao vivo') && t.includes('Pedidos') && t.includes('https://loja.dev/pedidos'));
must('painel mostra url, modo com porta e snapshot com ref na frente', t.includes('página   Pedidos') && t.includes('oculto · ao vivo') && t.includes('porta    9401') && /e1\s+heading "Pedidos"/.test(t));
...
must('cabeçalho: url ao vivo, modo, estado, ligação', t2.includes('loja.dev/pedidos/novo') && t2.includes('oculto') && t2.includes('ao vivo') && t2.includes('ligado a api') && t2.includes('Novo pedido'));
must('moldura da página ao vivo e refs com o ref na frente', t2.includes('página ao vivo') && t2.includes('o agente vê') && /e2\s+link "Novo"/.test(t2));
must('teclas: clique, digitar, abrir janela', t2.includes('i digitar') && t2.includes('o abrir janela') && t2.includes('clique na página'));
...
renderBrowser(g3, br, { live: { frame: null, url: '', title: '', error: '', connected: false }, box: { x: 2, y: 4, cols: 85, rows: 20 }, typing: true, canImg: true, booting: 'subindo o Chrome…' }, '', []);
const t3 = g3.toString();
must('sem frame mostra o que está acontecendo', t3.includes('subindo o Chrome…') && t3.includes('sem agente ligado'));
must('modo digitar avisa e troca as teclas', t3.includes('digitando na página') && t3.includes('sair do modo digitar'));
...
must('sem imagem, explica e mostra o snapshot com refs', t4.includes('não desenha imagens') && t4.includes('[ref=e2]'));
...
must('árvore mostra o agente no browser sem o prefixo', rs.some((r) => r.name === 'navigate' && r.detail === 'https://loja.dev') && rs.some((r) => r.name === 'click' && r.detail.includes('Novo')));
console.log(fails ? `\n${fails} falha(s)` : '\ntudo verde');
