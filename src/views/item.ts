...
import { C, G, ago, fit, pad, plain } from '../tui/theme.ts';
...
function shell(g: Grid, title: string, titleColor = C.inkHi) {
  surface(g, `anthive / ${title}`, '', titleColor);
}
...
function mdLines(g: Grid, ls: MdLine[], scroll: number, top: number, bottom: number) {
  ...
  scroll = Math.max(0, Math.min(scroll, ls.length - view));
  ...
  scrollHint(g, g.H - 2, scroll, Math.max(0, ls.length - scroll - view));
  return view;
}
...
function lines(g: Grid, ls: string[], scroll: number, top: number, bottom: number, numbered = false) {
  ...
  for (let i = 0; i < slice.length; i++) {
    if (numbered) g.put(2, top + i, String(scroll + i + 1).padStart(numW), C.dim);
...
export function renderNote(g: Grid, d: Doc, scroll: number, status: string, links: string[]) {
  shell(g, `${G.note} ${fit(plain(d.title), g.W - 30)}`, C.link);
  ...
  const view = body ? mdLines(g, renderMd(body, g.W - 4), scroll, 3, g.H - 3) : lines(g, [t('(empty)')], scroll, 3, g.H - 3);
  keybar(g, g.H - 1, [['↑↓', t('scroll')], ['e', t('edit in $EDITOR')], ['l', t('link')], ['d', t('delete')], ['esc', t('back')]], status);
  return view;
}
...
export function renderFile(g: Grid, f: FileItem, content: string[] | null, scroll: number, status: string, links: string[]) {
  ...
  const view = content && isMd ? mdLines(g, renderMd(content.join('\n'), g.W - 4), scroll, 3, g.H - 3)
    : lines(g, content ?? [t('(could not read — deleted, binary or no permission)')], scroll, 3, g.H - 3, !!content);
  keybar(g, g.H - 1, f.context
...
export function renderTask(g: Grid, n: TaskNode, agentName: string, status: string, links: string[]) {
  ...
  g.put(2, g.H - 3, fit(t('Managed by the agent · progress updates automatically'), g.W - 4), C.dim);
  keybar(g, g.H - 1, [['l', t('link')], ['esc', t('back')]], status);
}
...
export function renderService(g: Grid, s: ServiceItem, st: Stats | null, alive: boolean, status: string, links: string[]) {
  ...
  let y = 3;
  for (const [k, v] of rows) { g.put(2, y, pad(k, 11), C.dim); g.put(14, y, fit(v, g.W - 16), C.ink); y++; }
  y++;
  g.put(2, y, fit(t('Logs are available in the terminal running this process.'), g.W - 4), C.dim);
  keybar(g, g.H - 1, [['k', alive ? t('stop (SIGTERM)') : t('already dead')], ['l', t('link')], ['d', t('remove from project')], ['esc', t('back')]], status);
}
...
export function renderBrowser(g: Grid, n: BrowserNode, s: BrowserScreen, status: string, links: string[]) {
  ...
  if (s.box) {
    ...
    g.put(2, top, fit(t('Page snapshot · use o to open a browser window'), w), C.dim);
    const lines = st.snapshot ? st.snapshot.split('\n').filter((l) => !l.startsWith('```') && !/^###/.test(l) && !/^- (Page (URL|Title)|Console):/.test(l)).filter((l) => l.trim()) : [t('No page snapshot yet. Link an agent with l to start browsing.')];
  ...
  if (s.box || s.canImg) {
    ...
    if (w >= 16) {
      const head = (y: number, t: string) => { g.put(refsX, y, fit(t + ' ', w), C.dim); g.put(refsX + [...t].length + 1, y, G.h.repeat(Math.max(0, w - [...t].length - 1)), C.dim); };
  ...
  keybar(g, g.H - 1, s.typing ? [['esc', t('stop typing')], ['↵', 'enter'], ['⌫', t('delete')], [t('click'), t('the page')]] : [[t('click'), t('the page')], ['↑↓', t('scroll')], ['i', t('type')], ['o', mode], ['r', t('reload')], ['l', t('link')], ['d', t('remove')], ['esc', t('back')]], status);
}
